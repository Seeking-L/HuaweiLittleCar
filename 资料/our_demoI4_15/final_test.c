...
/*这个时间是小车在没有障碍行驶的时候所能
  行驶的最大时间，超过这时间就要往目的地走*/
#define CONTINUE_TIME 2

/*控制pwm延长时间，也就是转向的角度大小*/
#define PWM_DELAY_TIME 90
...
typedef enum{    //这里是显示小车的速度方向
    DIR_X,
    DIR_Y
}DIRECTION;

typedef enum{    //这里是显示小车的速度方向正负
    NEG,
    POS
}NEG_OR_POS;
...
int expect_speed_state[2] = {0, 0}; 
int current_speed_state[2] = {0, 0};

//这是距目的地距离
float distance_to_destination = 0.0;

float start_position[2]={0.0,0.0};

float destination_position[2]={X0,0.0};//这里假设以目的地和起始地的连线为x轴

float current_position[2]={0.0,0.0};

static volatile int buttonState = 0;  //按键被按下的标志
...
void change_speed_state()
{
    ...
    car_stop();
    //!!! 是否需要判断x或y
    if(current_speed_state[DIR] == DIR_Y &&
    current_speed_state[PTR]==POS && 
    ...
    if(current_speed_state[DIR] == DIR_X &&
    current_speed_state[PTR]==POS && 
    expect_speed_state[PTR]==POS)
    {
        car_right();
...
{   
    int current_time=hi_get_seconds();
    int delta_time = current_time - last_start_time;  //运动的时间
    float delta_s = SPEED_VALUE * delta_time;
...
void find_destination()
{
    ...
    // distance_to_destination = 0;
     //这是记录接下来小车的方向
    int next_speed[2] = {0, 0};
    // if(abs(distance_x)<=abs(distance_y))
    float tmp_x = distance_x, tmp_y = distance_y;
    if (distance_x < 0) tmp_x = -distance_x;
    if (distance_y < 0) tmp_y = -distance_y;
    if (tmp_x <= tmp_y)
    {
L_X:
        next_speed[DIR] = DIR_X;
        if(distance_x<0)
            next_speed[PTR] = NEG;
        else
            next_speed[PTR] = POS;
        if(current_speed_state[DIR]==next_speed[DIR] && current_speed_state[PTR]!=next_speed[PTR])//这个是检测小车是不是原路返回了
        {
            goto L_Y;
        }
        }
...
L_Y:
    ...
    //获取期望状态
    expect_speed_state[DIR] = next_speed[DIR];
    expect_speed_state[PTR] = next_speed[PTR];
}
