...
// 这是距目的地距离,以米为单位
volatile float distance_to_destination_power = X0 * X0;
...
volatile float distance_x = 0.0; // 当前距离目的地的距离
volatile float distance_y = 0.0;
...
void get_current_position(float last_position[], int last_start_time)
{
    ...
    // 距离目的地距离X方向大小
    distance_x = destination_position[DIR_X] - current_position[DIR_X];
    // 距离目的地距离Y方向大小
    distance_y = destination_position[DIR_Y] - current_position[DIR_Y];
    // 总的距离大小
    distance_to_destination_power = (distance_x * distance_x + distance_y * distance_y);
...
int car_start()
{
    ...
    printf("Trip started!    ");
    printf("Distance to destination'power:%f\n", distance_to_destination_power);
...
void FinalTestTask()
{
    ...
    while (distance_to_destination_power > DISTANCE_LIMIT)
    {
        ...
        printf("TIME:%d      , Distance to destination:%f\n", current_time, distance_to_destination_power);
        printf("(%f,%f)   ", current_position[DIR_X], current_position[DIR_Y]);
        if (expect_speed_state[DIR] == DIR_X)
            if (expect_speed_state[PTR] == POS)
                printf("expect dir:X+   ");
            else
                printf("expect dir:X-   ");
        else
        ...
        if (current_speed_state[DIR] == DIR_X)
            if (current_speed_state[PTR] == POS)
                printf("curr speed:X+\n");
            else
                printf("curr speed:X-\n");
        else
